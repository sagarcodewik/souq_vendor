...
const UpdateProfile = () => {
  ...
  const initialValues = {
    ...
    licenseDocument: vendor.licenseDocument || null,
  ...
  const saveProfile = async (values, setSubmitting) => {
    ...
    const validCoords =
      ...
      coords.length === 2 &&
      coords.every((n) => typeof n === 'number' && !isNaN(n) && n !== 0)

    if (!validCoords) {
      alert('Location not detected. Please enable GPS or refresh.')
      setSubmitting(false)
      return
    }
    ...
    <div className={styles.container}>
      <div className={styles.wrapper}>
        <Formik
          ...
          onSubmit={(vals, helpers) => saveProfile(vals, helpers.setSubmitting)}
        >
